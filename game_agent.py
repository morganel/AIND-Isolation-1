...
def return_winner_loser(game, player):
...
def linear_combination_moves_score(game, player):
    ...
    At the beginning of the game, "max_blanks-blanks" is small and "blanks"
    is high. A higher absolute coefficient is applied to the opponent's moves,
    which translates into a more aggressive style.

    At the end of the game, blanks is lower and max_blanks-blanks is high.
    A higher coefficient is applied to the player's moves, which translates
    into a more defensive style.
    ...
    wl = return_winner_loser(game, player)
...
def moves_and_position(game, player):
    """
    Based on number of moves available and each player's distance
    to the center of the board.

    10 * (own_moves - opp_moves) +
    (own_dist_x + own_dist_y) - (opp_dist_x + opp_dist_y)

    Calculates the sum of the absolute number of squares from
    the player's position to the center of the board along the x and y axis.
    This heuristics puts a positive coefficient to the player's distance
    to center, pushing it towards the edges.
    """
...
def moves_and_position_opposite(game, player):
    ...
    Calculates the sum of the absolute number of squares from
    the player's position to the center of the board along the x and y axis.
    This heuristics puts a negative coefficient to the player's distance
    to center, pushing it towards the center.
    """
...
class CustomPlayer:
    ...
    def negamax(self, game, depth, maximizing_player=True):
        ...
        if (game.utility(player_to_maximize) != 0):
            return maximizing_player * game.utility(player_to_maximize),
            bestMove

        if (depth == 0):
            return maximizing_player * self.score(game, player_to_maximize),
            bestMove
        ...
        bestMove = legalMoves[0]
        for m in legalMoves:
            v, _ = self.negamax(game.forecast_move(m),
                                depth - 1, -maximizing_player)
            v = -v
            if v > bestValue:
                bestValue = v
                bestMove = m
